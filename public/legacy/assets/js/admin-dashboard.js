...
const popupNote=document.getElementById('wa-runtime-note');popupNote?.insertAdjacentHTML('afterend',`<div id="wa-popup-setting" class="rounded border border-gray-200 bg-white p-3 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><label class="flex items-start gap-2 text-sm"><input id="wa-popup-enabled" type="checkbox" class="mt-1 accent-green-600" onchange="updateWhatsAppPopupSetting(this.checked)"><span><b>Storefront WhatsApp signup popup</b><small class="block text-xs text-gray-500 mt-1">Keep this off while testing. Turn it on when you want visitors to see the number and ₹100 offer form.</small></span></label><span id="wa-popup-setting-msg" class="text-xs text-gray-500">Loading...</span></div>`);loadWhatsAppPopupSetting();updateWhatsAppCampaignPreview();
}
...
async function cancelWhatsAppCampaign(id){if(!confirm('Cancel every pending job in this campaign? Already-sent messages cannot be recalled.'))return;const{ok,body}=await adminFetch(`/api/admin/whatsapp/campaigns/${id}/cancel`,{method:'POST'});if(!ok)return alert(body.error||'Could not cancel campaign');await loadWhatsAppCampaigns();}
...
const recipientLabel = document.getElementById('email-recipient-count')?.previousElementSibling;
if (recipientLabel) recipientLabel.textContent = 'Verified customers';
const campaignCopy = document.querySelector('#email-campaign-section p.tiny.text-gray-500');
if (campaignCopy) campaignCopy.textContent = 'Only verified, non-admin Eternal Essence customer accounts are eligible.';
...
(function () {
  function renderWhatsAppFailureDetails() {
    const campaigns = typeof whatsappCampaignState === 'object' ? whatsappCampaignState?.campaigns || [] : [];
    const cards = [...document.querySelectorAll('#wa-campaign-list article')];
    campaigns.forEach((campaign, index) => {
      const errors = campaign.errors || [];
      const card = cards[index];
      if (!card || !errors.length || card.querySelector('.wa-failure-details')) return;
      const details = document.createElement('div');
      details.className = 'wa-failure-details mt-3 rounded border border-red-200 bg-red-50 p-2 text-xs text-red-800';
      details.innerHTML = `<b>Meta failure reason:</b> ${errors.map(error => escapeHtml(error.message || 'Unknown error')).join('<br>')}`;
      card.appendChild(details);
    });
  }

  const observer = new MutationObserver(renderWhatsAppFailureDetails);
  observer.observe(document.body, { childList: true, subtree: true });
